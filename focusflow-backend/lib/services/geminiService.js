...
class GeminiService {
  ...
  async callGeminiStructured(systemPrompt, userContent, options = {}) {
    ...
    const maxRetries = options.maxRetries || 3; // 🆕 增加重試次數
    let lastError = null;
...
5. If the response is getting cut off, prioritize completing the JSON structure over adding more content

For subtasks schema, ensure each subtask contains ALL of these required fields:
- id: A unique string identifier (e.g., "subtask_001", "subtask_002")
- title: A clear, descriptive title
- text: Detailed description (50-100 words)
- aiEstimatedDuration: Estimated time in minutes (number between 30-120)
- difficulty: Exactly one of "easy", "medium", "hard"
- order: Sequential number starting from 1
- completed: Boolean false
- skills: Array of skill tags (can be empty [])
- recommendedResources: Array of resource suggestions (can be empty [])
- phase: Exactly one of "knowledge", "practice", "application", "reflection", "output", "review"

IMPORTANT: If you cannot complete the full response within token limits, prioritize completing the JSON structure over adding more content.`;
        ...
        try {
          ...
          try {
            ...
            parsedResponse = this.repairTruncatedJSON(responseText);
            console.log(`✅ JSON repair successful`);
            parsedResponse._repaired = true;
          } catch (repairError) {
            console.error(`❌ JSON repair failed:`, repairError);
            
            // 檢查是否為截斷響應的跡象
            const possibleTruncationIndicators = [
              responseText.length < 100, // 響應太短
              responseText.includes('...'), // 包含省略號
              !responseText.includes('}'), // 沒有閉合大括號
              responseText.match(/,\s*$/), // 以逗號結尾
              responseText.match(/:\s*"[^"]*$/), // 不完整的字符串
            ];
            ...
            if (isTruncated && attempt < maxRetries) {
              ...
              lastError = new Error(`Truncated JSON response: ${parseError.message}`);
              continue;
            } else {
        ...
        // 🆕 驗證響應結構
        const validationResult = this.validateStructuredResponse(parsedResponse, schemaType);
        if (!validationResult.isValid) {
          console.warn(`⚠️ Schema validation failed:`, validationResult.errors);
          
          // 🆕 嘗試修復常見問題
          const repairedResponse = this.repairStructuredResponse(parsedResponse, schemaType);
          if (repairedResponse) {
            console.log(`🔧 Attempting to repair response structure...`);
            const revalidation = this.validateStructuredResponse(repairedResponse, schemaType);
            if (revalidation.isValid) {
              console.log(`✅ Response repair successful`);
              repairedResponse._repaired = true;
              return repairedResponse;
            }
          }
          
          if (attempt < maxRetries) {
            console.log(`🔄 Retrying with enhanced constraints...`);
  ...
  /**
   * 🆕 修復截斷的 JSON 響應
   * @param {string} responseText - 原始響應文本
   * @returns {object} - 修復後的 JSON 對象
   */
  repairTruncatedJSON(responseText) {
    console.log('🔧 Attempting to repair truncated JSON...');
    ...
    if (openBraces > closeBraces) {
      console.log(`🔧 Detected ${openBraces - closeBraces} missing closing braces`);
      ...
      repairedResult._originalLength = responseText.length;
