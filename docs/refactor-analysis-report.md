...
☒ Phase 1A: 建立測試基礎設施 - 設置 Jest/Vitest 單元測試環境，建立測試工具和 mocks       
     ☒ Phase 1B: 配置外部化重構 - 建立 AppConfig 系統，移除硬編碼配置，撰寫配置測試
     ☐ Phase 1C: 統一錯誤處理框架 - 建立 ErrorHandler 類別，替換分散的錯誤處理邏輯
     ☐ Phase 4: Playwright MCP 整合測試 - 執行完整的端對端測試，驗證重構後的功能
     ☐ Phase 2A: Schema 工廠模式重構 - 重構 GeminiService 的 Schema 管理，建立動態 Schema 生成
     ☐ Phase 2B: 統一 AI 服務層 - 建立 AIService 單例，移除重複的生成函數
     ☐ Phase 2C: 任務處理器統一化 - 重構 JobQueueService，建立通用任務處理器框架
     ☐ Phase 3A: UI 邏輯分離 - 從 add-task.tsx 抽取 AI 邏輯到服務層
     ☐ Phase 3B: API 層重構完成 - 移除重複函數，統一使用 Job API
     ☐ Phase 5: 效能驗證與最佳化 - 測量重構前後的效能差異，確保沒有退化
---
